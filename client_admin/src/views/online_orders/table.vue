		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/online_orders/table','add') || $check_action('/online_orders/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/online_orders/table','del') || $check_action('/online_orders/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','dish_name')" min-width="200">
					...
					<el-table-column prop="categorization_of_dishes" @sort-change="$sortChange" label="菜品分类"
				v-if="user_group == '管理员' || $check_field('get','categorization_of_dishes')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','order_date')" min-width="200">
					...
					<el-table-column prop="customer_information" @sort-change="$sortChange" label="顾客信息"
				v-if="user_group == '管理员' || $check_field('get','customer_information')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_customer_information(scope.row['customer_information']) }}
				</template>
					</el-table-column>
					<el-table-column prop="customer_name" @sort-change="$sortChange" label="顾客姓名"
				v-if="user_group == '管理员' || $check_field('get','customer_name')" min-width="200">
					</el-table-column>
					<el-table-column prop="customer_gender" @sort-change="$sortChange" label="顾客性别"
				v-if="user_group == '管理员' || $check_field('get','customer_gender')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','contact_number')" min-width="200">
					...
					<el-table-column prop="order_quantity" @sort-change="$sortChange" label="点餐数量"
				v-if="user_group == '管理员' || $check_field('get','order_quantity')" min-width="200">
					</el-table-column>
					<el-table-column prop="total_price_of_dishes" @sort-change="$sortChange" label="菜品总价"
				v-if="user_group == '管理员' || $check_field('get','total_price_of_dishes')" min-width="200">
					</el-table-column>
					<el-table-column prop="order_notes" @sort-change="$sortChange" label="点餐备注"
				v-if="user_group == '管理员' || $check_field('get','order_notes')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/online_orders/table','set') || $check_action('/online_orders/view','set') || $check_action('/online_orders/view','get') || $check_action('/${table.tableConfig.table_nav_name}/table','add') || $check_action('/${table.tableConfig.table_nav_name}/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/online_orders/table','set') || $check_action('/online_orders/view','set') || $check_action('/online_orders/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/online_orders/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/online_orders/get_list?like=0",
				url_del: "~/api/online_orders/del?",
		...
		methods: {
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						let sqlwhere = "(";
																																			if(user_group=="顾客用户"){
						sqlwhere+= "customer_information = " + this.user.user_id + " or ";
					}
			...
			get_user_customer_information(id){
				...
				return ret;
																							...
																							openPayModal(v){
				this.pay_obj.id = v.online_orders_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/online_orders/set?online_orders_id="+this.pay_obj.id);
