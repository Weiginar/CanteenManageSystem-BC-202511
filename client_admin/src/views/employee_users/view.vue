		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="员工姓名" prop="employee_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['employee_users_id'] && $check_field('set','employee_name')) || (!form_sub['employee_users_id'] && $check_field('add','employee_name'))"></el-input>
				...
				<el-form-item label="员工性别" prop="employee_gender">
						...
						v-if="user_group === '管理员' || (form_sub['employee_users_id'] && $check_field('set','employee_gender')) || (!form_sub['employee_users_id'] && $check_field('add','employee_gender'))">
				...
				<el-form-item label="员工工号" prop="employee_id">
							  ...
							  v-if="user_group === '管理员' || (form_sub['employee_users_id'] && $check_field('set','employee_id')) || (!form_sub['employee_users_id'] && $check_field('add','employee_id'))"></el-input>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["employee_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?employee_users_id=" + form_sub["employee_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/employee_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_users/view','get');
