<template>
	<div class="diy_details_box diy_div_dish_information">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/offline_orders/edit','add')" @click="to_form('/offline_orders/edit')" > 线下点餐 </button>
										<button class="btn btn-primary details_btn" v-if="$check_action('/online_orders/edit','add')" @click="to_form('/online_orders/edit')" > 线上点餐 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/dish_information/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/dish_information/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/dish_information/set?dish_information_id=' + _this.obj["dish_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.dish_name
																																		query.img = this.obj.picture_of_dishes
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var dish_information_id = this.obj.dish_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "dish_information",
          source_field: "dish_information_id",
          source_id: dish_information_id
        }, (res) => {
