		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐菜品信息模块(开始) -->
		<div class="warp" v-if="$check_action('/dish_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/dish_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_dish_information = !show_list_dish_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_dish_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 菜品信息推荐列表组件 -->
		                <list_dish_information v-if="show_list_dish_information" :list="list_dish_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_dish_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_dish_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_dish_information" :key="i" @click="to_details('dish_information',o,'dish_information_id')">
		                            <td v-for="(oj,n) in fields_dish_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
				import list_dish_information from "@/components/diy/list_dish_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_dish_information: true,
				list_dish_information: [],
				vm_arr_dish_information: [
					"dish_information_id",
							"dish_name",
				...
				fields_dish_information: [
						...
						type: "文本"
													...
													{
						key: "picture_of_dishes",
						label: "菜品图片",
						type: "图片"
					},
													{
						key: "introduction_to_dishes",
						label: "菜品介绍",
						type: "编辑"
					},
									],
		...
		methods: {
			...
			get_dish_information() {
				let url = "~/api/dish_information/get_list?";
								// url = "~/api/dish_information/get_list?orderby=hits desc";
								this.$get(url, {
									"orderby": "hits desc",
							"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_dish_information = json.result.list;
					}
				})
			},
		...
		computed: {
					    list_table_dish_preferences() {
		        var list = this.list_dish_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_dish_information.map((o) => {
		                // 第二个中括号是对象的属性
